...
void setHuman() {
  ...
  uint32_t darkYellow = strip.Color(150, 255, 0);
  uint32_t red = strip.Color(255, 0, 0);
  strip.fill(red);
  strip.setPixelColor(3, 0, 0, 0, 0);
  strip.setPixelColor(6, brightYellow);
  strip.setPixelColor(9, brightYellow);
  strip.setPixelColor(10, brightYellow);
  strip.setPixelColor(13, brightYellow);
  strip.setPixelColor(14, brightYellow);
  strip.show();
}
...
void setup() {
  strip.begin();
  strip.show();
//  EEPROM.write(EEPROM_ADDR, TRIGGERED_VALUE);         //Set Original Zombie
